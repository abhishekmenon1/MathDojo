...
from utils.constants import db_imp, db_sub
...
class SubtractionPage(Screen):
    ...
    def fadein_img(self):  # animation for the background image
        fadein = Animation(opacity=0.8, duration=4.5)
    ...
    def scorean_switch(self, *args):
        ...
        switch2 = Animation(line_color=(255 / 255, 255 / 255, 255 / 255, 1), duration=0.5)
    ...
    def partial_set3(self, *args):
        ...
        secfade = Animation(line_color=[255 / 255, 255 / 255, 255 / 255, 1], md_bg_color=[0, 0, 0, 0.4], duration=0.5)
        # secfade2 = Animation(line_color=[248/255, 181/255, 12/255, 1],  duration=2)
        secfade2 = Animation(line_color=[255 / 255, 255 / 255, 255 / 255, 1], duration=2)
    ...
    def subtraction(self):  # method for the addition game, to get random numbers
        self.progressbar.value = 100
        ...
        if self.top_num.text != "" and self.bot_num.text != "":
            if self.answer.text != "":
                ...
                alpha = int(self.top_num.text) - int(self.bot_num.text)
                if apple is True:  # change made
                    self.summation = int(self.top_num.text) - int(self.bot_num.text)
                    if self.summation == int(self.answer.text) and self.button_check is True:
                    ...
                    elif self.summation != int(self.answer.text) and self.button_check is True:
                        self.streak = 0
                        self.depression()
                        self.top_nd_bot(self.top_num.text, self.bot_num.text, self.answer.text, self.summation)
                        self.total_question = self.total_question + 1
                        self.scorered()
                        self.scorered_card()
                    ...
                    #     self.streak = 0
                    #     self.depression()
                    #     self.top_nd_bot(self.top_num.text, self.bot_num.text, self.answer.text, sum)
                    #     self.total_question = self.total_question + 1
                    #     self.scorered()
                    #     self.scorered_card()
                    ...
                    self.total_question = self.total_question + 1
                    self.top_nd_bot(self.top_num.text, self.bot_num.text, "non-number", alpha)
                    self.what.text = "What?"
                    self.numberfamo()
                    self.scorered()
                    self.scorered_card()
                ...
                self.streak = 0
                self.depression()
                subtract = self.topnum - self.botnum
            #                self.top_nd_bot(self.top_num.text, self.bot_num.text, "-", subtract)
    ...
    def the_questions(self):
        ...
        elif 2 <= self.streak < 4:  # change made
            i = randint(25, 45)
            self.top_num.text = str(i)
            j = randint(12, 24)
            self.bot_num.text = str(j)

        elif 4 <= self.streak < 7:  # change made
            i = randint(55, 85)
            self.top_num.text = str(i)
            j = randint(31, 54)
            self.bot_num.text = str(j)

        elif 7 <= self.streak < 9:  # change made
            i = randint(99, 150)
            self.top_num.text = str(i)
            j = randint(40, 69)
            self.bot_num.text = str(j)

        elif 9 <= self.streak < 15:  # change made
            i = randint(130, 200)
            self.top_num.text = str(i)
            j = randint(101, 129)
            self.bot_num.text = str(j)

        elif self.streak >= 15:  # change made
            i = randint(210, 350)
            self.top_num.text = str(i)
            j = randint(160, 209)
            self.bot_num.text = str(j)
    ...
    def percent(self):  # method to add information on popup screen, percentage, time taken
        if self.total_question > 0:
            ...
            if db_imp.new_subtract_ans(self.tim):
                self.seconds_anim()
    ...
    def stfire(self, fire):  # method to show the highest streak the player had in the popup box
        ...
        self.a_streak = max(self.fire_list)

        if 3 <= self.a_streak < 5:
    ...
    def popupbox(self, hel):  # method to add popup screen
        ...
        if self.ctdown2 == 0:  # at this time the popup box will open up
            ...
            if self.a_streak > 0:
                db_sub.add_info(self.score.text, str(self.a_streak), self.the_seconds.text)
                db_imp.compare_value_sub(self.score.text, str(self.a_streak), self.the_seconds.text)
            else:
                db_sub.add_info(self.score.text, '0', self.the_seconds.text)
                db_imp.compare_value_sub(self.score.text, '0', self.the_seconds.text)
    ...
    def scorechange(self):  # method for the add the score
        ...
        if db_imp.new_subtract_score(score):
            if score < 10:
                self.score_anim()
    ...
    def when_opened(self):
        ...
        # self.back_card2.line_color = (248/255, 181/255, 12/255, 0)
        # self.back_card3.line_color = (248/255, 181/255, 12/255, 0)
        self.back_card.line_color = (1, 1, 255 / 255, 0)
        self.back_card2.line_color = (1, 1, 1, 0)
        self.back_card3.line_color = (1, 1, 1, 0)
