...
from utils.constants import db_imp, db
...
class AdditionPage(Screen):
    ...
    def expandingc(self):
        ...
        self.next.disabled = True
        val = Animation(opacity=0, value=100, duration=0.1)
        opac = Animation(opacity=0, duration=0.05)
    ...
    def addition(self):  # method for the addition game, to get random numbers
        self.progressbar.value = 100
        ...
        if self.top_num.text != "" and self.bot_num.text != "":
            if self.answer.text != "":
                ...
                self.streak = 0
                join = self.topnum + self.botnum
                self.top_nd_bot(self.top_num.text, self.bot_num.text, '-', join)
                self.depression()
    ...
    def popupbox(self, hel):  # method to add popup screen
        ...
        if self.ctdown2 == 0:  # at this time the popup box will open up
            ...
            if self.a_the_streak > 0:  # CHANGE MADEEEEEEE
                db.add_info(self.score.text, str(self.a_the_streak), self.the_seconds.text)
                db_imp.compare_values_addition(self.score.text, str(self.a_the_streak), self.the_seconds.text)
            else:
                db.add_info(self.score.text, '0', self.the_seconds.text)
                db_imp.compare_values_addition(self.score.text, '0', self.the_seconds.text)
    ...
    def scorechange(self):  # method for the add the score
        ...
        if db_imp.new_addition_score(score):
            if score < 10:
                self.score_anim()
