...
from utils.constants import db_imp, db_per


class PercentPage(Screen):
    ...
    top_num_str = ObjectProperty(None)
    the_seconds_new = ObjectProperty(None)
    streak_label = ObjectProperty(None)
    ...
    def fadein_img(self):  # animation for the background image
        fadein = Animation(opacity=1, duration=4.5)
    ...
    def scorean_switch(self, *args):
        #        switch2 = Animation(line_color=(248 / 255, 181 / 255, 12 / 255, 1), duration=0.5)
        switch2 = Animation(line_color=(218 / 255, 255 / 255, 131 / 255, 1), duration=0.5)
    ...
    def motivation(self):
        list = ['assets/images/ko.png', 'assets/images/monk.png', 'assets/images/ok.png', 'assets/images/easy.png',
                'assets/images/firec.png',
    ...
    def expand2(self, *args):
        # anim = Animation(size=(1280/10, 720/10), duration=0.15)
        # anim.start(self.visual)
    ...
    def depression(self):
        list = ['assets/images/no1.png', 'assets/images/no2.png', 'assets/images/no3.png', 'assets/images/no4.png',
                'assets/images/no5.png']
        a = random.choice(list)
    ...
    def partial_set3(self, *args):
        ...
        secfade = Animation(line_color=[218 / 255, 255 / 255, 131 / 255, 1], md_bg_color=[0, 0, 0, 0.4], duration=0.5)
        #        secfade2 = Animation(line_color=[248/255, 181/255, 12/255, 1],  duration=2)
        secfade2 = Animation(line_color=[214 / 255, 255 / 255, 117 / 255, 1], duration=2)
    ...
    def top_nd_bot(self, top, bot, userans, realans):
        ...
        for value, i in enumerate(self.ttop):
            ...
            for value_two, j in enumerate(self.bbot):
                ...
                if value == value_two:
                    continue
        joined = first_num, "% of ", second_num
    ...
    def the_percent_set(self):  # method for the percentage game, to get random numbers
        self.progressbar.value = 100
        self.question_timer(1)
        self.answer.focus = True
        self.correct.text = ""
        if self.top_num.text != "" and self.bot_num.text != "":
            alpha = float(self.bot_num.text) * float(self.top_num_str) / 100
            if self.answer.text != "":
                orange = self.answer.text
                try:
                    float(orange)
                    if alpha == float(self.answer.text) and self.button_check is True:
                    ...
                    if alpha != float(self.answer.text) and self.button_check is True:
                        self.streak = 0
                        self.depression()
                        self.top_nd_bot(self.top_num_str, self.bot_num.text, self.answer.text, alpha)
                        self.total_question = self.total_question + 1
                        self.scorered()
                        self.scorered_card()
                except ValueError:
                    self.total_question = self.total_question + 1
                    self.top_nd_bot(self.top_num_str, self.bot_num.text, orange, alpha)
                    self.what.text = "What?"
                    self.numberfamo()
                    self.depression()
                    self.scorered()
                ...
                self.streak = 0
                self.depression()
                self.top_nd_bot(self.top_num_str, self.bot_num.text, '-', alpha)
            self.percent_process()
        else:
            self.percent_process()

    def percent_process(self):
        self.operator.text = "of"
        self.operator.color = (248 / 255, 181 / 255, 12 / 255, 1)
        self.operator.bold = True
        init_num = [1, 10, 50, 100]
        init_num2 = [20, 25, 30, 40, 75]
        init_num3 = [7, 9, 12, 8, 4]
        if self.streak < 3:
            i = random.choice(init_num)
            self.top_num_str = str(i)
            lister = [self.top_num_str, "%"]
            self.top_num.text = ''.join(lister)
            binit_num = [5, 10, 15, 20, 25, 30]
            j = random.choice(binit_num)
            self.bot_num.text = str(j)
            self.topnum = i
            self.botnum = j

        elif 3 <= self.streak < 5:  # change made
            i = random.choice(init_num)
            self.top_num_str = str(i)
            lister = [self.top_num_str, "%"]
            self.top_num.text = ''.join(lister)
        ...
        elif 5 <= self.streak < 9:  # change made
            i = random.choice(init_num2)
            self.top_num_str = str(i)
            lister = [self.top_num_str, "%"]
            self.top_num.text = ''.join(lister)
            j = randint(11, 39)
        ...
        elif 9 <= self.streak < 11:  # change made
            i = random.choice(init_num2)
            self.top_num_str = str(i)
            lister = [self.top_num_str, "%"]
            self.top_num.text = ''.join(lister)
        ...
        elif 11 <= self.streak < 13:  # change made
            i = random.choice(init_num3)
            self.top_num_str = str(i)
            lister = [self.top_num_str, "%"]
            self.top_num.text = ''.join(lister)
            j = randint(40, 89)
    ...
    def stfire(self, fire):  # method to show the highest streak the player had in the popup box
        ...
        if 2 < self.a_the_streak < 5:
            if db_imp.new_percent_streak(self.a_the_streak) is True:  # continue from here, animation pane set up
                self.seconds_anim_streak()
                self.streaknum.text = str(max(self.fire_list))
            else:
                self.streaknum.text = str(max(self.fire_list))

            comp_one = ["percent percent,[i] percent![/i]", "Keep Practicing, Keep Improving.", "Good start.",
            ...
            compliments = ["Now, lets cross [i]Streak-15[/i]", "Woah good effort,[i] buddy![/i]",
    ...
    def popupboxinfo(self):  # method for what the score popup box will show
        ...
        self.back_card.line_color = (218 / 255, 255 / 255, 131 / 255, 0)
        self.back_card2.line_color = (218 / 255, 255 / 255, 131 / 255, 0)
        self.back_card3.line_color = (218 / 255, 255 / 255, 131 / 255, 0)
        ...
        if db_imp.new_percent_streak(self.a_the_streak) == False:
            self.streaknum.opacity = 1
        else:
            self.streaknum.opacity = 0
    ...
    def popupbox(self, hel):  # method to add popup screen
        ...
        if self.ctdown2 == 0:  # at this time the popup box will open up
            ...
            if self.a_the_streak > 0:  # CHANGE MADEEEEEEE
                db_per.add_info(self.score.text, str(self.a_the_streak), self.timbertime)
                db_imp.compare_values_percentage(self.score.text, str(self.a_the_streak), self.timbertime)
            else:
                db_per.add_info(self.score.text, '0', str(self.timbertime))
                db_imp.compare_values_percentage(self.score.text, '0', str(self.timbertime))

        if hello is True:
            if self.total_question > 0:  # if the player decided to leave the game mid way
                self.quit_popup()
                self.no_ques.text = "'Every expert was once \na beginner.'"
                self.no_ques.font_size = '19sp'
                self.no_ques2.text = "-Rutherford Hayes"
                self.no_ques2.font_size = '14sp'
    ...
    def question_timer(self, timepassed):
        ...
        if self.progressbar.value == 0:
            reg_sum = float(self.bot_num.text) * float(self.top_num_str) / 100
            if self.answer.text != "":
                try:
                    float(self.answer.text)
                    if float(self.answer.text) == reg_sum:
                        ...
                        if self.streak < 2:
                            ...
                            self.scorean(5)
                        self.the_percent_set()
                    else:
                        self.total_question = self.total_question + 1
                        self.top_nd_bot(self.top_num.text, self.bot_num.text, self.answer.text, reg_sum)
                        self.streak = 0
                        self.the_percent_set()
                        self.clear()
                        self.depression()
                    ...
                    self.total_question = self.total_question + 1
                    print("top num:", self.top_num.text)
                    print("bot num:", self.bot_num.text)
                    self.top_nd_bot(self.top_num.text, self.bot_num.text, self.answer.text, reg_sum)
                    self.streak = 0
                    self.the_percent_set()
                    self.clear()
                ...
                # self.top_nd_bot(self.top_num.text, self.bot_num.text, "-", reg_sum)
                self.total_question = self.total_question + 1
                self.streak = 0
                self.the_percent_set()
                self.clear()
